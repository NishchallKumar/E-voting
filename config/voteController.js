...
exports.castVote = async (req, res) => {
  ...
  try {
    ...
    if (election.status !== 'active') return res.status(400).json({ msg: 'Voting is not active' });
    ...
    // Increment candidate's vote count (atomic operation)
...
exports.getResults = async (req, res) => {
  ...
  try {
    const election = await Election.findById(electionId);
    if (!election) return res.status(404).json({ msg: 'Election not found' });
    if (election.status !== 'ended') return res.status(400).json({ msg: 'Results not available yet' });

    const candidates = await Candidate.find({ electionId }).sort({ voteCount: -1 });
    const totalVotes = candidates.reduce((sum, c) => sum + c.voteCount, 0);
    const winner = candidates[0] || null;

    res.json({ election, candidates, totalVotes, winner });
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server error');
  }
